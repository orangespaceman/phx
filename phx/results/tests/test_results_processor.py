...
def fake_result(parkrun: Boolean = False) -> tuple:
    return (
        ...
        fake.date(),
        "parkrun" if parkrun else fake.random_element(["5K", "10K", "HM", "M"]),
        fake.word(),
        fake.city(),
        fake.random_int(),
        fake.random_int(),
...
class TestProcessResults(TestCase):
    ...
    def test_fetch_results_returns_most_recent_results(self):
        _first_file = File.objects.create(file="BrightonPhoenix_2024-05-05.xlsx")
        second_file = File.objects.create(file="BrightonPhoenix_2024-05-12.xlsx")
    ...
    def test_process_creates_new_sheet_with_new_results(self):
        ...
        with patch("results.results_processor.load_workbook") as mock_load_workbook:
            mock_load_workbook.side_effect = [fake_results(15), fake_results(10)]
            ...
            self.assertEqual(6, len(list(processor.results["NEW RESULTS"].rows)))
    ...
    def test_process_populates_new_sheet_with_expected_results(self):
        ...
        with patch("results.results_processor.load_workbook") as mock_load_workbook:
            ...
            mock_load_workbook.side_effect = [curr, prev]
    ...
    def test_save_creates_new_file(self):
        ...
        with patch("results.results_processor.load_workbook") as mock_load_workbook:
            mock_load_workbook.side_effect = [fake_results(10), fake_results(10)]
